...
while ($entry = readdir(REPIN)) {
	...
	if ($tree) {
      ...
      my $nodoc = $root->getElementsByTagName('id')->[0]->getFirstChild->getData;
			...
			$titre =~ s/\s+/ /g;
			$titre =~ s/ +/ /sg;
			$titre =~ tr/a-zA-Z0-9 //dc;
      my @motstitle = split(" ", $titre);
      foreach my $mottitle (@motstitle) {
				if (!exists($hashwords->{$mottitle})) {
					#print "mot1 : $mot\n";
					my %hashtf = ();
          my @tabtf = (1,0,0);
					$hashtf{$nodoc} = \@tabtf;
					$hashwords->{$mottitle} = \%hashtf;
				} else {
					#print "mot2 : $mot\n";
					$hashwords->{$mottitle}{$nodoc}[0]++;
				}
				#print "$mot | ";
			}
      ...
      $category =~ s/\s+/ /g;
      $category =~ s/ +/ /sg;
      $category =~ tr/a-zA-Z0-9 //dc;

      my @motscat = split(" ", $category);
      foreach my $motcat (@motscat) {
				if (!exists($hashwords->{$motcat})) {
					#print "mot1 : $mot\n";
					my %hashtf = ();
          my @tabtf = (0,1,0);
					$hashtf{$nodoc} = \@tabtf;
					$hashwords->{$motcat} = \%hashtf;
				} else {
					#print "mot2 : $mot\n";
					$hashwords->{$motcat}{$nodoc}[1]++;
				}
				#print "$mot | ";
			}
